...
class Dispatch(object):
    ...
    def updateElevState(self):
        ...
        for i in range(len(self.messages)):
            ...
            if self.elev.anim_state[i] == PE:
                ...
                self.elev.stateshow[i].setStyleSheet("QGraphicsView{border-image: url(:/state.png)}")
                continue
            ...
            if len(self.messages[i]):
                ...
                if self.elev.elev_state[i] == STILL:
                    ...
                    if self.elev.elev_floor[i] < self.messages[i][0]:
                        ...
                        if self.state[i] == RUNNING_DOWN:
                            ...
                            outbtn.setStyleSheet("QPushButton{border-image: url(:/down.png)}"
                            ...
                            outbtn.setStyleSheet("QPushButton{border-image: url(:/up.png)}"
                ...
                # 若电梯在运动
                    ...
                    if self.elev.elev_floor[i] < floor: # 向上运动
                        ...
                        self.elev.stateshow[i].setStyleSheet("QGraphicsView{border-image: url(:/state_up.png)}")

                    elif self.elev.elev_floor[i] > floor: # 向下运动
                        self.elev.elev_state[i] = RUNNING_DOWN
                        self.elev.elev_floor[i] -= 1
                        self.elev.led[i].setProperty("value", self.elev.elev_floor[i])
                        self.elev.stateshow[i].setStyleSheet("QGraphicsView{border-image: url(:/state_down.png)}")
    ...
    def warnCtrl(self, idx):
        ...
        if self.elev.door_state[idx] != OPEN:
            ...
            self.elev.stateshow[idx].setStyleSheet("QGraphicsView{border-image: url(:/state.png)}")
