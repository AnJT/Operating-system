    ...
    def updateElevState(self):
        for i in range(len(self.messages)):
            ...
            if len(self.messages[i]):
                ...
                self.state = RUNNING_DOWN if self.staet == RUNNING_UP else RUNNING_UP
    ...
    def elevMove(self, elev_idx, floor_idx):
        ...
        if now_floor < floor_idx: # 当前层小于按键
            ...
            if self.elev.elev_state[elev_idx] == STILL:
                ...
                if self.elev.elev_state[elev_idx] == RUNNING_UP:
                    self.messages[elev_idx].append(floor_idx)
                    self.messages[elev_idx].sort()
                elif self.elev.elev_state[elev_idx] == RUNNING_DOWN:
                    self.messages_reverse[elev_idx].append(floor_idx)
                    self.messages_reverse[elev_idx].sort()
            ...
            if self.elev.elev_state[elev_idx] == STILL:
                ...
                if self.elev.elev_state[elev_idx] == RUNNING_DOWN:
                    self.messages[elev_idx].append(floor_idx)
                    self.messages[elev_idx].sort()
                    self.messages[elev_idx].reverse()
                elif self.elev.elev_state[elev_idx] == RUNNING_UP:
                    self.messages_reverse[elev_idx].append(floor_idx)
                    self.messages_reverse[elev_idx].sort()
                    self.messages_reverse[elev_idx].reverse()
